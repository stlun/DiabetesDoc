...
/**
 * This Class provides methods to create pdf output.<br>
 * The input is taken from the XML-files in the directory <i>xml</i>}.
...
final class OutputCreator {
   ...
   */
  public static void createPDF(Calendar startDate, Calendar endDate) throws IOException, COSVisitorException {
   ...
   */
  public static void createPDF(Calendar startDate, final Calendar endDate, final ProgressMonitor pm) throws IOException, COSVisitorException {
    File out = new File("pdf", Utils.toDateString(startDate) + "_" + Utils.toDateString(endDate) + ".pdf");
    out.getParentFile().mkdirs();

    // Create a new document
    PDDocument pdfDoc = new PDDocument();
    PDRectangle pageSize = new PDRectangle(PDPage.PAGE_SIZE_A4.getHeight(), PDPage.PAGE_SIZE_A4.getWidth()); // A4 - transverse
    PDPage page;
  ...
  private static void addPageHeader(PDPageContentStream pdfStream) throws IOException {
    ...
    pdfStream.drawLine( MARGIN_SIZE + FIRST_CELL_WIDTH, 19.0f*CM, MARGIN_SIZE + FIRST_CELL_WIDTH, 19.7f*CM);
    ...
    pdfStream.drawString( DiabetesDoc.getSetting("username") + "  " + DiabetesDoc.getSetting("birthday") );
    ...
    drawCenteredString(pdfStream, Utils.localize("%output.overview%"), (Table.MAX_COLS + 1) * CELL_WIDTH, 12);
   ...
   */
  static void drawCenteredString(final PDPageContentStream contentStream, final String txt, final float width) throws IOException {
   ...
   * @throws IOException If an I/O exception occurs while getting the width of the String.
   */
  static void drawCenteredString(final PDPageContentStream contentStream, final String txt, final float width, final float fontSize)
      throws IOException {
    float centering;
    try {
