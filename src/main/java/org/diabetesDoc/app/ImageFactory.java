...
final class ImageFactory {
  ...
  /** The template image */
  private static javax.swing.ImageIcon template = new javax.swing.ImageIcon(ImageFactory.class.getResource("/template.png"));
   ...
   * @throws java.io.IOException If an I/O exception occurs while reading the XML-String.
   * @throws org.jdom2.JDOMException If an JDOM exception occurs while parsing the XML-String.
   */
  public static BufferedImage getImage(final String xmlTxt) throws java.io.IOException, org.jdom2.JDOMException {
   ...
   * Creates an image of the given XML-document.
   *
   * @param xmlDoc the XML-{@link org.jdom2.DocumentDocument} with DTD DAY (see <i>resources/day.dtd</i>).
   * @return The {@link BufferedImage} of the day's data.
   */
  public static BufferedImage getImage(final org.jdom2.Document xmlDoc) {
    String timeStr[];
    int time, bz, posX, posY;

    BufferedImage img = new BufferedImage(template.getIconWidth(), template.getIconHeight(), BufferedImage.TYPE_INT_RGB);
    Graphics g = img.getGraphics();
    g.drawImage(template.getImage(), 0,0, template.getImageObserver());
    ...
    for(org.jdom2.Element e : xmlDoc.getRootElement().getChildren()) {
      if(e.getName().equals("BG") && !e.getAttributeValue("Val").equals("---")) {
        ...
        posY = Y_OFF - ( Math.min(Math.max(0, bz - BG_MIN), BG_MAX - BG_MIN) * DIAGRAM_HEIGHT )
            / (BG_MAX - BG_MIN);
        if(bz < BG_MIN) {
          g.fillPolygon(new int[]{ posX - 10, posX, posX + 10 }, new int[]{ posY - 10, posY, posY - 10 }, 3);
        } else if(bz > BG_MAX) {
          g.fillPolygon(new int[]{ posX - 10, posX, posX + 10 }, new int[]{ posY + 10, posY, posY + 10 }, 3);
        } else {
          g.fillPolygon(new int[]{ posX - 4, posX - 10, posX    , posX + 10, posX + 4, posX + 10, posX    , posX - 10 },
                        new int[]{ posY    , posY - 10, posY - 4, posY - 10, posY    , posY + 10, posY + 4, posY + 10 }, 8);
        }
        g.drawString("" + bz, Math.max( X_OFF + 10, Math.min( posX-30, X_OFF + DIAGRAM_WIDTH - 60)),
